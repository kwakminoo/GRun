...
const SECTION_TITLE_CLASS = "text-3xl md:text-4xl lg:text-5xl font-bold text-center text-neutral-800";
    ...
    <section className="relative w-full min-h-[100vh] flex flex-col">
      <header className="fixed top-0 left-0 right-0 z-30 flex items-center justify-between px-6 md:px-12 py-[5px] text-white bg-black/30 backdrop-blur-sm border-b border-white">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm">
          <a href="#why-we-run" className="hover:underline">Why We Run</a>
          <a href="#calendar" className="hover:underline">캘린더</a>
        </nav>
        <a
        ...
        <div className="relative min-h-[6rem] md:min-h-[8rem] flex items-center justify-center w-full max-w-4xl">
          {HERO_TEXTS.map((item, i) => (
            ...
            >
              ...
              <p className="text-base md:text-xl text-white/90 whitespace-nowrap">{item.sub}</p>
            </div>
          ))}
        </div>
...
const RUNNING_COURSES = [
  { name: "광화문", img: "https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?w=900&q=80", key: "gwanghwamun" },
  { name: "북촌", img: "https://images.unsplash.com/photo-1566576912321-d58ddd7a6088?w=900&q=80", key: "bukchon" },
  { name: "경리단길", img: "https://images.unsplash.com/photo-1517154421773-0529f29ea451?w=900&q=80", key: "gyeongridan" },
];
  ...
  return (
    <section id="calendar" className="min-h-[100vh] w-full flex flex-col bg-white">
      <div className="container mx-auto px-6 md:px-12 flex-1 flex flex-col justify-center py-6 md:py-8">
        <h2 className={`${SECTION_TITLE_CLASS} mb-4 md:mb-6 text-2xl md:text-3xl`}>
          캘린더
        </h2>
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-10 items-stretch justify-center max-w-7xl mx-auto">
                        ...
                        <td
                          ...
                          className={
                            ...
                            today.getFullYear() === year
                              ? "border border-neutral-300 align-top text-left bg-neutral-800 text-white font-medium"
                              : `border border-neutral-300 align-top text-left bg-white text-neutral-800 ${j === 0 ? "text-red-600" : ""} ${j === 6 ? "text-blue-600" : ""}`
                          }
                          style={{ height: "6.5rem", minHeight: "6.5rem" }}
                        >
                          {d !== 0 ? (
          ...
          <div className="w-full lg:w-[16.5rem] flex flex-col gap-3 min-h-0">
            {RUNNING_COURSES.map((course) => (
              <div key={course.key} className="flex-1 min-h-0 flex flex-col rounded-xl overflow-hidden shadow-lg bg-white border border-neutral-200">
                <div className="flex-1 min-h-0 min-h-[4.5rem] overflow-hidden relative">
                  <Image
                    src={course.img}
                    alt={course.name}
                    fill
                    className="object-cover"
                    sizes="(max-width: 768px) 100vw, 16.5rem"
                  />
                </div>
...
export default function Home() {
  return (
    <main>
      ...
      <WhyWeRunSection />
      <CalendarSection />
      <footer id="contact" className="py-8 bg-neutral-800 text-white text-center text-sm">
        <p>그런 GRun · 문의하기</p>
      </footer>
    </main>
  );
